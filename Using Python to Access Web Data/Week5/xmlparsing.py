...
#and extract the comment counts from the XML data, compute the sum 
...
#Sum=2553)
#Actual data: http://py4e-data.dr-chuck.net/comments_307980.xml (Sum 
#ends with 46)
#You do not need to save these files to your folder since your 
#program will read the data directly from the URL. Note: Each 
...
#values sum the numbers. The closest sample code that shows how to 
#parse XML is geoxml.py. But since the nesting of the elements in 
#our data is different than the data we are parsing in that sample 
#code you will have to make real changes to the code.
#To make the code a little simpler, you can use an XPath selector 
#string to look through the entire tree of XML for any tag named #'count' with the following line of code:

#counts = tree.findall('.//count')
#Take a look at the Python ElementTree documentation and look for 
#the supported XPath syntax for details. You could also work from 
#the top of the XML down to the comments node and then loop through #the child nodes of the comments node.
#Sample Execution
...
url = 'http://py4e-data.dr-chuck.net/comments_307980.xml'
